...
_F1_RULES = [
    FirewallRule(
        ...
        family="F1",
        pattern=re.compile(
            r"\b(?:ignore|disregard|forget|override|bypass)\b"                          # Cerca una parola chiave del comando rispettando i confini della parola (es: non si attiva per forgotten) 
    ...
    FirewallRule(
        ...
        family="F1",
        pattern=re.compile(r"\b(?:system|assistant)\s*:\s*\S", re.IGNORECASE),          # Cerca una parola che definisce il ruolo com system o assistant
   ...
   FirewallRule(
    ...
    FirewallRule(
        ...
        family="F1",
        pattern=re.compile(r"\bpretend\s+(?:you\s+are|to\s+be)\b", re.IGNORECASE),      # Tentativo di jailbreak
...
RULES: list[FirewallRule] = [*_F1_RULES, *_F2_RULES] # Concatene le famiglie di regole in un'unica lista